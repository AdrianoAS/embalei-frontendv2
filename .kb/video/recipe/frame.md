...
description: >
  Video-first companion to Code editorial's design.md, remixed onto the Embalei brand. The unit is
  the frame (1920×1080). The app is a LIGHT packing-station UI, so the palette is inverted from the
  original preset: a near-white neutral ground (#f6f6f7), pure-white cards (#ffffff), a hairline
  border (#e6e7eb), near-black ink (#11181c), and a single HeroUI blue (#006fee) as the scarce
  "voltage". The code/terminal surface is a real dark slate (#0f172a / #0b1220 / #1e293b), taken
  from the app's own dark pieces — the login visual, the label animation and the toast. Elevation
  stays hairline (no heavy shadow). Typography is the house's, NOT the app's: Lato for display and
  body, JetBrains Mono for the index/code voice, because those woff2 ship with this recipe.
  Sentence-case display and the ✱ accent spike mark are kept. Composition + frame scale rewritten.
  Motion out of scope.
...
typography:
  # — reading + chrome ramp —
  body:    { fontFamily: "Lato", cqw: 1.5, weight: 400, lineHeight: 1.5 }
  lead:    { fontFamily: "Lato", cqw: 2.08, weight: 400, lineHeight: 1.5 }
  card-title:{ fontFamily: "Lato", cqw: 2.3, weight: 400, lineHeight: 1.25, tracking: "-0.005em" }
  button:  { fontFamily: "Lato", cqw: 1.46, weight: 400, lineHeight: 1.0 }
  tag-upper:{ fontFamily: "Lato", cqw: 1.35, weight: 400, tracking: "0.18em", upper: true }
  ...
  # — display ramp (Lato 400, sentence case, negative tracking. Renderer embeds only 400/700 — author at 400; italic is the synthesized slant) —
  headline:{ fontFamily: "Lato", cqw: 4.6, weight: 400, lineHeight: 1.06, tracking: "-0.018em" }
  quote-pull:{ fontFamily: "Lato", cqw: 5.0, weight: 400, lineHeight: 1.12, tracking: "-0.012em", italic: true }
  display-italic:{ fontFamily: "Lato", cqw: 6.7, weight: 400, lineHeight: 1.05, tracking: "-0.012em", italic: true }
  display:{ fontFamily: "Lato", cqw: 7.3, weight: 400, lineHeight: 1.02, tracking: "-0.022em" }
  number-hero:{ fontFamily: "Lato", cqw: 9.4, weight: 400, lineHeight: 0.95, tracking: "-0.025em" }
  display-cover:{ fontFamily: "Lato", cqw: 9.9, weight: 400, lineHeight: 0.98, tracking: "-0.028em" }
...
components:
  ...
  number-lockup:
    ...
    description: "Hero stat — a Lato figure paired with a JetBrains Mono unit (200K, +1,204, −318, 17 files). Figure is serif; the unit is ALWAYS mono, never the serif."
  pull-quote:
    typography: "{typography.quote-pull} (Lato italic) + {typography.tag-upper} cite"
    description: "A commit message, a reviewer line, or the thesis. Lato italic, small Lato uppercase cite beneath."
  section-rule:
...
This is the **code-editorial** preset remixed onto the **Embalei** brand. The YAML frontmatter above (colors · typography · components) is **normative and already correct — use it verbatim.** The prose below is the ORIGINAL preset's intent; read it THROUGH the frontmatter:

- **The app is LIGHT.** Embalei is a packing station running on a fixed tablet in the DC: `src/app/layout.tsx` hardcodes `class="light" data-theme="light"` and there is not a single `prefers-color-scheme: dark` block in `src/app/globals.css`. So the original preset's light/dark polarity holds here — unlike the dark app this recipe was first built for.
- **Colors** — use the frontmatter hex; preset color NAMES in prose ("cream", "coral", "navy") mean the remapped Embalei values. Every one of them comes from the `:root` block of `src/app/globals.css`: `cream` = `--bg` `#f6f6f7` (the screen floor), `tile` = `--panel` `#ffffff` (the card), `tile-strong` = `--line` `#e6e7eb` (the hairline), `ink` = `--ink` `#11181c` (the text), `coral` = `--brand` `#006fee` (the one accent — HeroUI's default blue, which this app kept and mapped onto `--accent`, `--focus` and `--link`).
- **"Never pure white / no pure black" does NOT apply.** That is the original preset's warm-paper rule. Here **`#ffffff` IS the card surface** — it is `--panel`, the real surface of every panel in the app. The ground is a cool near-white, not warm cream; ignore "warm", "cream paper" and "no cool gray" wherever the prose insists on them.
- **The navy code surface is a real app surface, not an invention.** `#0f172a` is the background of the toast and of the label animation (`.toast`, `.gif--media`), `#0b1220` is the base of the login's dark column, `#1e293b` is the lighter step at the end of the label-animation gradient — used here for the title bar / status strip, which must read *above* the panel body on a light frame.
- **Fonts — the app's are NOT the video's.** Embalei renders **Inter** (400/500/600/700/800) for everything and **JetBrains Mono** for SKU, NF-e key and temperature (`src/app/globals.css:1` imports both from Google Fonts; `body` is Inter, `.mono` is JetBrains Mono). The video keeps **Lato** + **JetBrains Mono** anyway, because those are the faces shipped as licensed woff2 in `fonts/` — swapping the family without swapping the files breaks the offline render. Colors follow the app; typography is the house's. The mono half matches by luck: the app really does set identifiers in JetBrains Mono, so mono there is fidelity.
- **Weights** — Lato ships `{100, 300, 400, 700, 900}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/800) in prose, even though the app itself uses 800 for its display.
...
Three editorial voices, each in its own face: **Lato** carries every display moment — covers,
headlines, pull-quotes, big stat numerals — at large display sizes with gentle negative tracking;
its **italic** is the expressive register. **Lato** carries body, leads, card titles, buttons, and
...
- **Lato** (sentence case, negative-tracked) for all display; **Lato** body/chrome; **JetBrains Mono** index + code.
...
- **Squint** — one Lato display moment dominates at 3–6× its neighbor.
...
- **Type** — Lato sentence-case display (negative-tracked); Lato 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code.
...
Two ramps. The **reading/chrome ramp** (Lato `body` 1.5cqw / `lead` 2.08cqw weight 400; JetBrains
Mono `kicker`/`mono-label` in px) carries copy + chrome; the **display ramp** (Lato `headline` 4.6cqw
...
- **Lato display is sentence case** (NOT title case, NOT uppercase), weight 400, negative-tracked (−0.012..−0.028em); reach for **italic** when the line is a stance or a definition. **Lato body** sentence case weight 400. **JetBrains Mono** kickers UPPERCASE 0.16em with the coral ✱. No uppercase serif, no sans headline, no serif label.
...
- **number-lockup** — Lato figure + mono unit (the PR `+N / −M`, `200K`, file counts). **pull-quote** — Lato italic + cite (a commit message / reviewer line).
...
### 1 · Cover (identity · move: oversized Lato · cream)

**Ground** `{colors.cream}`, `slide-pad`. **Composes** kicker-spike, display-cover, lead, mono-label index. **Focal** a 2–3 line Lato `display-cover` (sentence case, ink) under a coral ✱ kicker. **Chrome** mono index strip (repo · branch). **Accent** the single coral ✱. **Fixed** Lato 400 sentence case, hairline, cream ground. **Free** title, the mono index, layout + how full the frame runs. **Density** free.

### 2 · Statement (statement · move: single Lato line · cream or navy)

**Ground** `{colors.cream}` (or `{colors.navy}` for gravity). **Composes** kicker-spike, display, optional lead. **Focal** one 2-line Lato `display` carrying the change in a sentence — reach for **italic** if it's a stance. **Chrome** mono kicker. **Accent** none — the serif carries it (or one coral word). **Fixed** sentence-case serif. **Free** the line, ground, layout + density. **Density** free.
...
- Set all display in **Lato, sentence case**, negative-tracked; **Lato 400** body; **JetBrains Mono** kickers (uppercase, 0.16em, coral ✱) + code.
...
- Pair a Lato figure with a **mono unit** for every stat; render code via the **`code-*` blocks** on the navy surface.
...
- No uppercase or title-case Lato display; no sans headline; no serif label; no serif-set numeric unit.
...
- **Squint** — one Lato display moment dominates.
...
- **Type** — Lato sentence-case display (negative-tracked); Lato 400 body; JetBrains Mono kickers (uppercase 0.16em, coral ✱) + code; ≥1.4cqw floor.
...
```html
<style>
@font-face{font-family:"Lato";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/Lato-400.woff2") format("woff2");}
@font-face{font-family:"Lato";font-weight:700;font-style:normal;font-display:block;src:url("assets/fonts/Lato-700.woff2") format("woff2");}
</style>
```
